(function ($) {
  ...
  // Date Picker
  if ($(".maru-datepicker").length) {
    $('.maru-datepicker').datetimepicker({
      timepicker: true,
      datepicker: true,
      format: 'y-m-d H:i',
      hours12: false,
  ...
  //accrodion
  if ($(".maru-accrodion").length) {
    var accrodionGrp = $(".maru-accrodion");
  ...
  $(".tab-btn").on("mouseenter", function () {
    $(".tab-btn").removeClass("active-btn");
    $(".tab").removeClass("active-tab").hide();

    $(this).addClass("active-btn");
    $($(this).data("tab")).addClass("active-tab").fadeIn();
  });
  ...
  function thmOwlInit() {
    // owl slider
    let maruowlCarousel = $(".maru-owl__carousel");
    if (maruowlCarousel.length) {
      maruowlCarousel.each(function () {
        let elm = $(this);
    ...
    let maruowlCarouselNav = $(".maru-owl__carousel--custom-nav");
    if (maruowlCarouselNav.length) {
      maruowlCarouselNav.each(function () {
        let elm = $(this);
  ...
  function maruSlickInit() {
    // slick slider
    let maruslickCarousel = $(".maru-slick__carousel");
    if (maruslickCarousel.length) {
      maruslickCarousel.each(function () {
        let elm = $(this);
        let options = elm.data("slick-options");
        let maruslickCarousel = elm.slick(
          "object" === typeof options ? options : JSON.parse(options)
        );
      });
    }
    let maruslickCarouselCounter = $(".maru-slick__custome-counter");
    if (maruslickCarouselCounter.length) {
      maruslickCarouselCounter.each(function () {
        let elm = $(this);
        let options = elm.data("slick-options");
        let currentSlide;
        let slidesCount;
        let sliderCounter = document.createElement('div');
        sliderCounter.classList.add('maru-slick__counter');

        let updateSliderCounter = function (slick, currentIndex) {
          currentSlide = slick.slickCurrentSlide() + 1;
          slidesCount = slick.slideCount;
          $(sliderCounter).html('<span class="maru-slick__counter__active">' + currentSlide + '</span>' + '' + '<span>' + slidesCount + '</span>')
        };
  ...
  //Strech Column
  function maru_stretch() {
    var i = $(window).width();
    $(".row .maru-stretch-element-inside-column").each(function () {
      var $this = $(this),
  ...
  $(window).on("load", function () {
    ...
    var input = document.querySelector("#country-phone-code-list");
    if (input) {
  ...
  for (let i = 0; i < link.length; i++) {
    link[i].addEventListener("mousemove", (e) => {
      ...
      linkHoverReveal[
        i
      ].style.transform = `translate(-100%, -50% ) rotate(29.2deg)`;
